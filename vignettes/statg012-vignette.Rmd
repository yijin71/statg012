...
The function `plot()` is a function to generate plotting of prior and posterior distributions of above six models. 

We will introduce how to use function `binombeta()` and function `normnorm()` individually, accompanied by function `summary()` and function `plot()` in the following parts.
...
We can notice that the contour plot of posterior looks like a "squashed egg". Compared with the prior contour, the posterior contour, after observing the data, is much more smoothly and narrow. 
...
Available from: www.researchgate.net/publication/238622435ACompendiumofConjugatePriors [Accessed10 August 2017].
